...
export function TrialModeSummary({ summaries }: TrialModeSummaryProps) {
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 w-full min-w-0" aria-label="โหมดและสถานะข้อมูลการปักชำกุหลาบ">
      {summaries.map((summary) => {
        ...
        const cardContent = (
          <div className="flex flex-col h-full justify-between p-4 md:p-5 space-y-4 min-w-0 w-full break-words">
            {/* Header */}
            <div className="space-y-2 min-w-0 w-full">
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400 font-medium leading-relaxed break-words">
                {summary.headline}
              </p>
            </div>

            {/* Details */}
            <div className="space-y-2 bg-neutral-50/50 dark:bg-neutral-900/30 rounded-xl p-3 border border-neutral-100 dark:border-neutral-900/40 min-w-0 w-full">
              {summary.details.map((detail: { label: string; value: string }, idx: number) => (
                <div key={idx} className="flex justify-between items-center text-xs gap-4 min-w-0 w-full">
                  <span className="text-neutral-400 dark:text-neutral-500 font-medium flex-shrink-0">{detail.label}</span>
                  <span className="text-neutral-800 dark:text-neutral-200 font-bold break-words text-right min-w-0">
                    {detail.value}
                  </span>
                </div>
              ))}
            ...
            {summary.warnings.length > 0 && (
              <div className="space-y-1.5 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-900/30 min-w-0 w-full">
                {summary.warnings.map((warning: string, wIdx: number) => (
                  <div key={wIdx} className="flex items-start gap-1.5 text-xs text-amber-700 dark:text-amber-400 font-semibold leading-relaxed break-words">
            ...
            {summary.href ? (
              <div className="flex items-center text-xs font-bold text-rose-600 dark:text-rose-400 group-hover:text-rose-700 pt-2 border-t border-neutral-100 dark:border-neutral-900 w-full">
                <span>เปิดดูรายละเอียด</span>
                <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
              </div>
            ) : (
              <div className="text-[10px] text-neutral-400 font-semibold pt-2 border-t border-neutral-100 dark:border-neutral-900 w-full">
        ...
        return summary.href ? (
          <Link
            key={summary.mode}
            href={summary.href}
            className={`group block rounded-2xl border ${borderClass} ${bgClass} hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 min-w-0 w-full`}
          >
            {cardContent}
          </Link>
        ) : (
          <div
            key={summary.mode}
            className={`rounded-2xl border ${borderClass} ${bgClass} min-w-0 w-full`}
          >
            {cardContent}
          </div>
        );
