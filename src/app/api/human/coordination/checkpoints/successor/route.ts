...
export async function POST(request: NextRequest) {
    try {
        ...
        const checkpoint = createGovernedSuccessorLaneCheckpoint(getDb(), body);
        ...
        if (error instanceof CoordinationLaneResolutionError) {
            const status =
                error.code === "COORDINATION_PROJECT_NOT_FOUND"
        ...
        if (error instanceof CoordinationCheckpointStaleSuccessorError) {
            return NextResponse.json(
                { ok: false, error: { code: error.code, message: error.message } },
                { status: 409 },
            );
        ...
        if (
            error instanceof CoordinationCheckpointError
            || error instanceof CoordinationCheckpointEvidenceError
        ) {
            return NextResponse.json(
                { ok: false, error: { code: error.code, message: error.message } },
                { status: 400 },
