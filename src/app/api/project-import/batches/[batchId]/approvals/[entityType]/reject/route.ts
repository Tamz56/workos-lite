...
import { rejectEntityApi } from "@/lib/project-import/approvalApplicationService";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ batchId: string; entityType: string }> }) {
    ...
    try {
        const actor: ImportActor = authenticateAndAuthorize(request, "project_import:reject");
        ...
        const approval = rejectEntityApi(batchId, entityType, actor.actorName, reason, { db: getDb() });
